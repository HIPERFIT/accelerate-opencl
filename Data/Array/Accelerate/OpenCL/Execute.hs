...
module Data.Array.Accelerate.OpenCL.Execute (
  ...
  -- * Execute a computation under a OpenCL environment
...
scanOp c kernel bindings acc aenv (Array sh0 in0) = do
  ...
  fadd                <- liftIO $ OpenCL.createKernel mdl "exclusive_update"
...
scan1Op c kernel bindings acc aenv (Array sh0 in0) = do
  (mdl,fscan,(t,g,m)) <- configure kernel acc (size sh0)
  fadd                <- liftIO $ OpenCL.createKernel mdl "inclusive_update"
  res@(Array _ out)   <- newArray c (toElt sh0)
  (Array _ bks)       <- newArray 1 (Z :. g) :: CIO (Vector e)
  (Array _ sum)       <- newArray 1 Z        :: CIO (Scalar e)
  let n   = size sh0
...
bindLifted :: OpenCL.Program -> Val aenv -> [AccBinding aenv] -> CIO ()
...
class Marshalable a where
  marshal :: a -> CIO [OpenCL.KernelArg]
...
instance Marshalable ty where {                                                \
  marshal x = return [OpenCL.VArg x] }
...
configure :: AccKernel a
          -> PreOpenAcc ExecOpenAcc aenv a
          -> Int
          -> CIO (OpenCL.Program, OpenCL.Kernel, (Int,Int,Integer))
...
dispatch :: Marshalable args
         => (OpenCL.Program, OpenCL.Kernel, (Int,Int,Integer))
         -> [AccBinding aenv]
         -> Val aenv
         -> args
         -> CIO ()
